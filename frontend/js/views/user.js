...
views['u-dash'] = async () => {
  ...
  if (adminVideo && adminVideo.url) {
    videoHtml = `
    <div style="background:var(--surface-2);border:1px solid var(--primary);border-left:4px solid var(--primary);border-radius:12px;padding:16px;margin-bottom:20px">
      <h4 style="color:var(--primary);margin-bottom:8px">🎬 ${adminVideo.title || 'رسالة من الكابتن'}</h4>
      <p style="font-size:12px;color:var(--text-dim);margin-bottom:12px">${adminVideo.date || ''}</p>
      <video controls style="width:100%; border-radius:8px; max-height:250px; background:#000; border:1px solid var(--border)">
        <source src="${adminVideo.url}">
...
views['u-nutrition'] = async () => {
  ...
  // جيب صورة النظام الغذائي لو موجودة
