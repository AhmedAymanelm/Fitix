...
router = APIRouter(prefix="/gym-settings", tags=["Gym Settings"])
...
def get_gym_settings(db: Session = Depends(get_db)):
    ...
    if not s:
        return {"gym_name": "FORM Fitness", "primary_color": "#c8ff3d", "logo_url": None}
    return {
        "gym_name": s.gym_name,
...
def update_gym_settings(payload: GymSettingsUpdate, db: Session = Depends(get_db)):
    ...
    db.commit()
    return {"status": "ok", "gym_name": s.gym_name, "primary_color": s.primary_color}
